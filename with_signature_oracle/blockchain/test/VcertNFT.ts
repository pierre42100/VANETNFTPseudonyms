...
describe("VcertNFT", function () {
  ...
  describe("Mint", function () {
    ...
    it("Invalid code", async function () {
      ...
      await expect(sc.mintNFT(INVALID_CODE, "addr", time())).to.be.revertedWith(
    ...
    it("From other accounts", async function () {
      ...
      expect(
        await sc.connect(otherAccount).mintNFT(VALID_CODE, "addr", time() + 300)
      );
    });
  });
  ...
  describe("Check", function () {
    ...
    it("Retrieve a NFT by its token id", async function () {
      ...
      const token = await sc.connect(owner).mintNFT(VALID_CODE, pubKey, time());
  ...
  describe("Set signature", () => {
    it("Un-existent token", async function () {
      ...
      await expect(
        sc.connect(owner).setSignature("sig", 10)
    ...
    it("Valid token", async function () {
      ...
      await sc.connect(owner).mintNFT(VALID_CODE, "mypubkey", time());

      expect(await sc.connect(owner).setSignature("sig", 1));
    });

    it("Set signature multiple times", async function () {
      const { sc, owner } = await loadFixture(deployFixture);

      await sc.connect(owner).mintNFT(VALID_CODE, "mypubkey", time());

      expect(await sc.connect(owner).setSignature("sig", 1));
      await expect(sc.connect(owner).setSignature("sig", 1)).to.be.rejectedWith(
        Error
      );
    });

    it("Other account", async function () {
      const { sc, owner, otherAccount } = await loadFixture(deployFixture);

      await sc.connect(owner).mintNFT(VALID_CODE, "mypubkey", time());

      await expect(
        sc.connect(otherAccount).setSignature("sig", 1)
      ).to.be.rejectedWith(Error);
    });
  });
  ...
  describe("Get signature", () => {
    it("Un-existent token", async function () {
      ...
      await expect(sc.connect(owner).getSignature(10)).to.be.rejectedWith(
        Error
      );
    });

    it("Signature unavailable", async function () {
      const { sc, owner, otherAccount } = await loadFixture(deployFixture);

      await sc.connect(owner).mintNFT(VALID_CODE, "mypubkey", time());

      await expect(sc.connect(otherAccount).getSignature(1)).to.be.rejectedWith(
        Error
      );
    });

    it("Signature available", async function () {
      const { sc, owner, otherAccount } = await loadFixture(deployFixture);
      ...
      expect(await sc.connect(otherAccount).getSignature(1)).to.be.equals(
        "sig"
      );
    });
  });
